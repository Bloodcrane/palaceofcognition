...
const AdminPage = () => {
    ...
    const fetchRecent = useCallback(async () => {
        ...
        try {
            ...
            const response = await databases.listDocuments(VOTES_DATABASE_ID, collectionId, [Query.orderDesc('$createdAt'), Query.limit(5)]);
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const collectionId = type === 'book' ? BOOKS_COLLECTION_ID : MOVIES_COLLECTION_ID;
            await databases.createDocument(VOTES_DATABASE_ID, collectionId, ID.unique(), { title, author, description, imageUrl });
            alert(`${type === 'book' ? 'წიგნი' : 'ფილმი'} წარმატებით აიტვირთა!`);
            setTitle(''); setAuthor(''); setDescription(''); setImageFile(null);
            fetchRecent();
        } catch (error) {
            ...
            <div className="user-container">
                <div className="profileContainer">
                    ...
                    <div className="profile-actions" style={{ marginTop: '20px' }}>
                        <button className={`post-toggle-btn ${type === 'book' ? 'active' : ''}`} onClick={() => setType('book')} style={{ marginRight: '10px', background: type === 'book' ? '#5b744d' : '' }}>წიგნები</button>
                        <button className={`post-toggle-btn ${type === 'movie' ? 'active' : ''}`} onClick={() => setType('movie')} style={{ background: type === 'movie' ? '#5b744d' : '' }}>ფილმები</button>
                    </div>
                </div>
                ...
                <div className="upload-section" style={{ display: 'block', opacity: 1 }}>
                    ...
                    <form onSubmit={handleUpload} className="upload-form">
                        <input type="text" placeholder="სათაური" value={title} onChange={e => setTitle(e.target.value)} required />
                        <input type="text" placeholder="ავტორი / რეჟისორი" value={author} onChange={e => setAuthor(e.target.value)} required />
                        <textarea placeholder="აღწერა" value={description} onChange={e => setDescription(e.target.value)} required />
