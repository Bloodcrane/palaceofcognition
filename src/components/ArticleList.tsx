...
const ArticleList = () => {
    ...
    const handleVote = async (articleId: string, type: 'like' | 'dislike') => {
        if (!user) {
                            ...
                            {currentArticles.map((article) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        whileHover={{ scale: 1.01, border: '1px solid rgba(255,255,255,0.4)', transition: { duration: 0.3 } }}
                                    >
                                        ...
                                        <div className="webComponent-overlay">
                                            ...
                                            <div className="webComponent-meta">
                                                {article.isDynamic && article.user_id ? (
                                                    <Link href={`/profile/${article.user_id}`} className="webComponent-author" style={{ textDecoration: 'none', color: '#598eff' }}>
                                                        {article.author}
                                                    </Link>
                                                ) : (
                                        ...
                                        <div className="compact-button-row">
                                            <Link href={`/article/${articleId}`} className="compact-button">
                                                🗞️ ნახვა
                                            </Link>
                                            <div className="vote-group-compact">
                                                <button className="compact-button like-btn" onClick={() => handleVote(articleId, 'like')}>
                                                    💚 {likes[articleId] || 0}
                                                </button>
                                                <button className="compact-button dislike-btn" onClick={() => handleVote(articleId, 'dislike')}>
